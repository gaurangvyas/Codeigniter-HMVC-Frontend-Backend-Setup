...
class Ion_auth_model extends CI_Model
{
		...
		if ($recheck !== 0)
		{
			...
			if ($last_login + $recheck < time())
			{
				$query = $this->db->select('userId')
								  ->where(array($this->identity_column => $this->session->userdata('identity'), 'status' => 'active'))
								  ->limit(1)
				...
				else
				{
					...
					{
						$this->session->unset_userdata(array($identity => '', 'id' => '', 'userId' => ''));
					}
					else
					{
						$this->session->unset_userdata(array($identity, 'id', 'userId'));
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			//print_r($this->bcrypt->verify($password,$hash_password_db->password));exit;
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select(''.$this->tables['users'].'.*,'.$this->tables['user_details_mst'].'.name')
		...
		if ($query->num_rows() === 1)
		  {
			...
			if ($password === TRUE)
			{
				...
				//print_r($this->session->userdata('userSession'));exit;
				return TRUE;
			}
		}
	...
	public function get_users_groups($id=FALSE)
	{
		...
		// if no id was passed use the current users id
		return $this->db->select($this->tables['user_groups'].'.groupId, '.$this->tables['groups'].'.groupName')
		                ->where($this->tables['user_groups'].'.'.$this->join['users'], $id)
		                ->join($this->tables['groups'], $this->tables['groups'].'.groupId='.$this->tables['user_groups'].'.groupId')
	...
	**/
	...
	public function set_session($user)
	{
		...
		//print_r($this->session->userdata('identity'));exit;
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', userId, email, last_login')
